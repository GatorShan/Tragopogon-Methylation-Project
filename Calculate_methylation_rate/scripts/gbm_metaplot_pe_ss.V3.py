...
def processSample( geneDict, cdsDict, allcPath, sampleName, numBins, numBinsStream, streamSize, methTypes, isPrint ):
	...
	for chrm in chrmList:
		...
		if fileExists == True:
			...
			# allcDictAll included all cytosine sites; will be used to calculate methylation rate in the upstream and downstream regions
...
# I added this function to include all cytosine sites
	...
	for gene in geneAr:
		...
		if strand == "+":
			...
			if streamSize != 0:
				...
				curUpVarAr.reverse()
				# I updated the following section, to correct the calculation for upstream and downstream regions
