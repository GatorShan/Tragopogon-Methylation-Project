library(ggplot2)
df_CHG = read.table("/blue/soltis/shan158538/Methylation/OutPut/DMR_heatmap/CHG_new/Filtered_sorted_all_reformatted.txt", sep="\t", header = TRUE)

# Convert Data Frame Column to Vector by Subsetting Data; and reverse the order
Order <- df_CHG[1:5933,"chr_pos"]
Order <- rev(Order)
...
CHG_plot <- ggplot(df_CHG, aes(x = comparison, y = chr_pos, fill = diff)) +
  ...
  scale_colour_gradient2(low = "deepskyblue3",
                         ...
                         midpoint = 0,
                         space = "Lab",
                         breaks = c(95, 25, 0, -25, -95),
                         labels = c("95%", "25%", "ns", "-25%", "-95%"),
                         name = "Difference",
                         na.value = "grey50",
                         guide = "colourbar",
                         aesthetics = "fill") +
